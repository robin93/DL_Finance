...
def DateToInt(row):
    D,M,Y = row['Day'],row['Month'],row['Year']
    date_object = datetime.datetime(Y,M,D)
    return date_object.toordinal()

print "Beginning to resolve the date column"
data.insert(2,'Day',pd.DatetimeIndex(data['Date']).day)
print "Resolving day values"
data.insert(2,'Month',pd.DatetimeIndex(data['Date']).month)
print "Resolving month values"
data.insert(2,'Year',pd.DatetimeIndex(data['Date']).year)
print "Converting to the ordinal values of dates"
data.insert(2,'DateNumber',data.apply(DateToInt,axis=1))
data.drop(['Day','Month','Year','Date'],inplace=True,axis=1)
